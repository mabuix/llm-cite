...
GEO/LLMO（生成AI検索での露出）を、感覚ではなく数字で見るための評価ハーネス。

> **位置づけ**: これはエンジニアが自分の手元で回すためのOSSツール。企業がチームでブランドを大規模にモニタリングするB2B SaaSではない（用途も規模も別物）。本業や契約先がGEO関連プロダクトに関わる場合は、この線引き（開発者向け・OSS・個人規模）を守ること。

## 特徴

- **オフラインで動く**: 既定は決定論的な `replay` プロバイダ。ネットワークもAPIキーも要らず、毎回同じ結果になる。学習・テストに向く。
